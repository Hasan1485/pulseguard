...
/**
 * The scoring stage of the pipeline: consumes raw transactions from RabbitMQ,
 * enriches them with rolling features from Redis, runs XGBoost inference, and
 * fans results out to metrics and the WebSocket dashboard.
 */
@Component
public class TransactionConsumer {
    ...
    public TransactionConsumer(FeatureStore featureStore, FraudScorer scorer,
                               PipelineMetrics metrics, SimpMessagingTemplate broker,
    ...
    public void onTransaction(Transaction txn) {
        ...
        CardFeatures features = featureStore.readAndUpdate(
                txn.cardId(), txn.id(), txn.amount(), nowMillis);
        ...
        ScoredTransaction scored = new ScoredTransaction(
                txn.id(), txn.cardId(), txn.amount(),
                probability, probability >= fraudThreshold, txn.label(),
                features.txnCount60s(), features.timeSinceLastSec(), features.amountRatio(),
    ...
    private void broadcast(ScoredTransaction scored) {
        if (scored.flagged()) {
            ...
            log.info("FRAUD flagged: txn={} card={} amount={} p={}",
                    scored.id(), scored.cardId(), scored.amount(),
