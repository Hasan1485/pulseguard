 ...
 *
 * <p>Per card it maintains a 60-second sliding window of transaction timestamps
 * (sorted set) plus lifetime aggregates (hash: count, amount sum, last-seen).
 * All commands for one transaction are pipelined into a single round trip; the
 * window count and aggregates are read <em>before</em> the current transaction
 * is folded in, exactly mirroring how the training features were computed.
 */
@Component
public class FeatureStore {
    ...
    /**
     * Reads the card's rolling features and then folds the current transaction
     * into the store, in one pipelined round trip.
     */
