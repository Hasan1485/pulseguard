...
/**
 * Full-stack pipeline test against real Redis and RabbitMQ (Testcontainers):
 * publishes raw transactions, waits for them to be consumed, feature-enriched,
 * and scored by the ONNX model, then verifies scores, feature-store state, and
 * latency accounting.
 */
@SpringBootTest
class PipelineIntegrationTest {
    ...
    void transactionsFlowThroughThePipelineAndGetScored() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            rabbitTemplate.convertAndSend(new Transaction(
                    "it-" + i, cardId, 10 + random.nextDouble() * 200, v, 0,
                    System.nanoTime()));
        }

        await().atMost(Duration.ofSeconds(30)).untilAsserted(() ->
                assertThat(metrics.snapshot().get("processed"))
                        .satisfies(p -> assertThat(((Number) p).longValue())
